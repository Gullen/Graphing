...
void drawEquation(){
    ...
    strokeWeight(2);    
    stroke(graph1Color[0], graph1Color[1], graph1Color[2]);
...
void Change(){
  int tempAxiesX = axiesNumberX;
  int tempAxiesY = axiesNumberY;
  try{
    ...
    axiesNumberX = tempAxiesX;
    axiesNumberY = tempAxiesY;
  }
  
  graph1Color[0] = int(cp5.get(Textfield.class, "Red").getText()); //<>//
  graph1Color[1] = int(cp5.get(Textfield.class, "Green").getText()); //<>//
  graph1Color[2] = int(cp5.get(Textfield.class, "Blue").getText()); //<>//
